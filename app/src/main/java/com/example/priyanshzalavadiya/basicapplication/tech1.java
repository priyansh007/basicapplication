...
public class tech1 extends AppCompatActivity {
    ...
    public void process(View v){
        if(v.getId()==R.id.but1) {
            Intent toy = new Intent("com.example.priyanshzalavadiya.basicapplication.tech2");
            startActivity(toy);
        }
        Intent intent=null,chooser=null;
        if(v.getId()==R.id.s1) {
            intent= new Intent(Intent.ACTION_SEND);
            Uri p= Uri.parse("android.resource://com.example.priyanshzalavadiya.basicapplication/drawable/"+R.drawable.tech1);
